...
# Ensure upload, processed, and extracted frames directories exist
...
def draw_face_rectangle(img):
    # Dummy coordinates for the face rectangle; replace with your face detection logic
    x, y, w, h = 50, 50, 100, 100  # Example coordinates
...
def predict():
    ...
    # Calculate the average prediction if it's a video
    if is_video:
        average_prediction = np.mean(predictions)
    else:
        average_prediction = predictions[0]

    # For images, use a threshold logic for classification
    threshold = 0.5  # Threshold adjusted for classification: Real if <0.5, Fake if >=0.5
    if is_video:
        # For video, apply threshold logic
        if average_prediction < threshold:
            label = 'Real'
            confidence = 1 - average_prediction  # Confidence is inverse for Fake/Real
        else:
            label = 'Fake'
            confidence = average_prediction
    else:
        # For images, apply the same logic
        if average_prediction < threshold:
            label = 'Fake'
